...
async function main() {
  ...
  try {
    ...
    const result = await client.callTool({
      name: "create-template", // The name of the tool to call
      arguments: {
        // The actual arguments for the 'create-template' tool, matching its Zod schema
        templateName: "testTemplate",
        sObject: "Account",
        recordCount: 10,
        outputFormats: ["csv", "json"],
    ...
    console.log(
      ...
      JSON.stringify(listTemplatesResult, null, 2)
    );
    ...
    console.log(
      " get-template-details result:",
      JSON.stringify(getDetailsResult, null, 2)
    );
  } catch (err: unknown) {
    ...
    if (transport && typeof transport.close === "function") {
      ...
      console.log(
        "Transport does not have a close method or is not initialized."
      );
    }
  }
}
